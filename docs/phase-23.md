# Phase 23 — Coder Context Management + Feature Decomposition
...
**Prerequisite phases:** 21 (Mem0 active), 22 (spec layer — provides the `features` list)
**Informs phases:** 26 (arbitration loop assumes coder can complete; decomposition helps)
...
class FeaturePipelineInput(BaseModel):
    task_id: int
    task_title: str
    spec_toml: str          # full spec from Phase 22
...
### Routing in `webhooks/spec.py` (Phase 22)
