...
- Phase 16 complete (voice dispatch working — all dispatch paths confirmed stable)
...
- MCP factory from Phase 14 working (UI wraps `POST /api/v1/mcp/register`)
...
View and manage registered MCPs (Phase 14 backend).
...
Backend: `GET /api/v1/mcp` (Phase 14 endpoint). Register form posts to `POST /api/v1/mcp/register`.
