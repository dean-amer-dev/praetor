...
- Phase 19 complete (Intelligent MCP Agent — `POST /api/v1/mcp/request` working)
...
The research agent finds `ghcr.io/flux159/mcp-server-kubernetes` (high confidence), the factory registers it, and the two instances below are deployed. This validates the full Phase 19 pipeline end-to-end.

## Implementation

### 20a — Deploy Read-Only Instance

Via Phase 19 intelligent agent (research → register):
