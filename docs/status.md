...
| 13 | OpenWebUI Integration | 🔵 In Progress | praetor-mcp + infra-mcp registered in LiteLLM configmap (k3s-dean-gitops #806); Mem0 wired to OpenWebUI (komodo-dean-gitops #54); praetor-mcp deploy pending dean-mcp #18 merge; system prompt pending manual setup |
| 14 | Agent Benchmarking & Eval | 🔵 In Progress | benchmark-worker + eval datasets + runner script built (praetor #39); pending: deploy benchmark-worker, run datasets, record baselines |
