...
| 22 | Spec Layer + Planning Conversation | 🔁 PR open | PRs: praetor#124, dean-mcp#27. spec.py: POST /api/v1/spec/execute (new_app→app_factory, fix_pr/modify_app→dispatch) + POST /api/v1/memory/search (planner-global). Coder: _parse_spec + spec-aware prompts + request_limit from spec + planner-global write. Reviewer+Research: planner-global write. dean-mcp: memory_search + execute_spec tools. 13 new tests, 279 total pass. |
